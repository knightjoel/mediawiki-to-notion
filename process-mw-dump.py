...
# Markdown via pandoc. It will also extract and output all embedded files
            ...
            if event == "end":
                if tag_name == "namespace":
                    ...
                    if ns_id == self._ns_id_from_name("Main"):
                        ...
                        print(
                            "WARNING: Skipping page {}:{} because its namespace '{}'"
                            " is implicitly ignored".format(
                                ns_name, parser.title, ns_name
                            )
                        )
                        continue
                    ...
                    print(
                        "Processing {}:{}".format(
                            self._ns_name_from_id(ns_id), parser.title
                        )
                    )
                    parser.prepare()
...
class FileParser(PageParser):
    ...
    def save(self, basedir):
        ...
        if self.encoding != "base64":
            raise ValueError(
                "{}: expected base64 encoding, got {}".format(self.title, self.encoding)
            )
        ...
        try:
            with open(os.path.join(savedir, self.filename), mode="bw") as f:
...
class WikitextParser(PageParser):
    def prepare(self):
        ...
        # Delete the Table of Contents '__TOC__' marker which I sometimes prefix
        # with a header.
        ...
        end_pattern = re.compile(r"^\S", re.ASCII)
    ...
    def save(self, basedir):
        ...
        try:
            ...
            print(
                "\tERROR: pandoc could not read {}: {}".format(self.title, e),
                file=sys.stderr,
            )
            return

        try:
            with open(os.path.join(savedir, self.filename + ".md"), mode="bw") as f:
                pandoc.write(doc, file=f, format="gfm", options=["--wrap=none"])
        except OSError as e:
            print(
                "\tERROR: unable to output {}: {}".format(self.filename, e),
                file=sys.stderr,
            )


def custom_prepare(text, title, namespace):
    if namespace != "Main":
        return None
    ...
    text = re.sub(r"{{Attention}}", "💡 ", text, flags=re.IGNORECASE)
    ...
    text = re.sub(
        r"{{CiscoCase\|(\d+)}}",
        r"[http://tools.cisco.com/ServiceRequestTool/query/QueryCaseSearchAction.do?method=doQueryByCase&caseType=ciscoServiceRequest&SRNumber=\1 \1]",  # noqa: E501
        text,
    )
    ...
    text = re.sub(r"{{href\|(\S+)\s+([^\|]+)\|(.+)}}", r"[\1 \2] (\3)", text)
    ...
    text = re.sub(r"{{leftoffat\|(.+)}}", r"<aside>💡 You left off at: \1</aside>", text)

    # Transform {{Msgid}} template.
    text = re.sub(
        r"{{Msgid\|(\S+)\|(.+)}}",
        r"[http://marc.info/?i=\1 \2]",
        text,
        flags=re.IGNORECASE,
    )
    ...
    text = re.sub(
        r"{{MSKB\|(\d+)\|(.+)}}", r"[http://support.microsoft.com/kb/\1 \2]", text
    )

    # Transform {{Needsclarification}} template.
    text = re.sub("{{Needsclarification}}", "⚠️  ", text, flags=re.IGNORECASE)

    # Transform {{Needswork}} template.
    text = re.sub("{{Needswork}}", "🚧 ", text, flags=re.IGNORECASE)
    ...
    text = re.sub(r"{{source\|(.+?)}}", r"(source: \1)", text, flags=re.IGNORECASE)
    ...
    text = re.sub(r"{{sourcelink\|(\S+)\|(.+?)}}", r"(source: [\1 \2])", text)
    ...
    text = re.sub(r"{{sourcelink\|(\S+)\s(.+?)\|.+?}}", r"(source: [\1 \2])", text)

    # Transform {{VMwareKB}} template.
    text = re.sub(
        r"{{VMwareKB\|(\d+)(?:\|(.+))?}}", r"[http://kb.vmware.com/kb/\1 \2]", text
    )
...
def main():
    ...
    parser.add_argument("xmlfile", type=str, help="the path to the XML dump file")
