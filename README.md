...
2. You run the `process-mw-dump.py` script from this project to read the XML dump and extract each page and embedded file (PDF, image, etc) to a directory on your computer.
...
8. An EventBridge rule is triggered by the "blocks ready" event and starts the `NotionUpload` state machine in AWS Step Functions. The state machine attempts to acquire a lock to ensure that only one instance of the state machine is doing work at a time. If it's unable to acquire the lock, it spins until the lock is acquired. If the state machine fails while it's holding the lock, the `Cleanup` state machine is invoked which cleans up dangling locks. With the lock acquired, the `NotionUpload` state machine runs the `UploadNotionBlocks` Lambda function.
